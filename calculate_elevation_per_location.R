...
#'
#' @param localization_data A data frame containing localization points with columns named \code{lon} and \code{lat}
...
#'
#' @return A data frame identical to \code{localization_data}, with an additional column \code{DEM_elevation}
...
calculate_elevation_per_location <- function(localization_data, dem_file = "DEM_Harod.tif") {
  # Load the DEM raster
  DEM <- raster(dem_file)
  ...
  # Prepare points as SpatialPoints, assuming localization_data has lon & lat columns
  points_sp <- SpatialPoints(localization_data[, c("lon", "lat")], proj4string = CRS("+proj=longlat +datum=WGS84"))
  ...
  localization_data$DEM_elevation <- elevations
