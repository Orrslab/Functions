...
#'
#' @param localization_data A data.table with localization info. Must contain columns:
#'   - `TAG`: Tag ID
#'   - `TIME`: UNIX timestamp
...
#'
#' @return The input `localization_data` with an additional column:
...
calculate_abs_avg_elevation_diff_between_location_and_participating_bs <- function(
  ...
  localization_data[, row_id := .I]
  
  # Join DEM_elevation to participating_base_stations
  merged <- merge(
    participating_base_stations,
    localization_data[, .(row_id, TAG, TIME, DEM_elevation)],
    by = c("TAG", "TIME"),
    all.x = TRUE
  )
  ...
  result <- merge(localization_data, avg_diffs, by = "row_id", all.x = TRUE)
