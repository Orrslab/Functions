...
FiltLoc0 <- RawLoc1[which(RawLoc1$Sun_angle<180),]
FiltLoc1 <- list()
for (tag in unique(FiltLoc0$TAG))
{FiltLoc1[[tag]] <- atl_unifiedFilter(FiltLoc0[which(FiltLoc0$TAG==tag),],stdLimit = 80,spdLimit = 25,spdSteps = 4,distLimit = 300,distSteps = 2,val1Limit = 0.75,ellipsMovangle = 15,ellipsMovRadius = 15,ellipsMovDist = 30 )
}
...
# timeBurst and smoothing ---------- (Michal)
    E <- timeGapBurst(C,secTol=17,minBurstFix=39,sampRate=8)
