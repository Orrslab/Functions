...
timeGapBurst <- function(data,secTol,minBurstFix,keepadditionalvars=T){
  require(dplyr)
  dataBurst <- data %>% 
    group_by(TAG) %>% 
    dplyr::mutate(dt=as.numeric(dateTime-lag(dateTime),unit='secs'),
           gap=(dt>secTol),
           gap=ifelse(is.na(gap),F,gap),
           timeGapBurst=cumsum(gap)) %>% 
    group_by(TAG,timeGapBurst) %>% # remove burst with pointInBurst lower than "minBurstFix"
    dplyr::mutate (pointInBurst=n())%>%
    dplyr::filter(pointInBurst > minBurstFix) %>%
    ungroup()
  if(!keepadditionalvars)
    dataBurst <- dataBurst %>% dplyr::select(c("X", "Y", "TIME","dateTime","timeGapBurst", "pointInBurst"))
  return(as.data.frame(dataBurst))
}
 # previous version that used amt
